    ...
    public class RaycastWeapon : GrabbableEvents {
        ...
        void checkEjectInput() {
            ...
            for (int x = 0; x < EjectInput.Count; x++) {
                if (InputBridge.Instance.GetGrabbedControllerBinding(EjectInput[x], thisGrabber.HandSide) || Input.GetKeyDown(KeyCode.JoystickButton12) || Input.GetKeyDown(KeyCode.JoystickButton3)) {
                    EjectMagazine();
                    break;
                }
            }
